...
async function controlApi(request: Request, env: RuntimeEnv): Promise<Response | null> {
  ...
  if (path === '/api/execution/callback' && request.method === 'POST') {
    ...
    const input = await body<{ jobId: string; status: string; result?: Record<string, never>; blockedReason?: string; currentStep?: number }>(request);
  ...
  const protectedWrite = request.method !== 'GET' && (
    ...
    path.startsWith('/api/decision-reviews/') ||
    path.startsWith('/api/notifications/') ||
    path === '/api/system-health/refresh' ||
  ...
  if (path.startsWith('/api/experiments/') && request.method === 'PATCH') {
    return json(await updateExperiment(env.DB, idFrom(path), await body(request)));
  }
  ...
  if (path === '/api/connectors/import' && request.method === 'POST') {
    const input = await body<{ connector: string; items: Array<{ externalId: string; title: string; body?: string; projectName?: string; occurredAt?: string; type?: string; tags?: string[]; importance?: number; metadata?: Record<string, never> }> }>(request);
